...
public class ExtendWNInterface extends InterfaceJavascript {
	...
	public int extendShouldOverrideUrlLoading(WebView view, String url) {
		...
		// 안드로이드 시뮬레이터에서 localhost:8080으로 요청되는 URL을 10.0.2.2:8080으로 변경
		if (url != null && url.startsWith("http://localhost:8080")) {
			String newUrl = url.replace("http://localhost:8080", "http://10.0.2.2:8080");
			view.loadUrl(newUrl);
			// URL 처리를 여기서 완료했으므로 모피어스나 다른 WebViewClient가 처리하지 않도록 함
			return InterfaceJavascript.URL_LOADING_RETURN_STATUS_TRUE;
		}
