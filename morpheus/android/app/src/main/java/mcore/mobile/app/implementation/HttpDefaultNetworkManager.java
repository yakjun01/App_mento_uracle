...
//			    [requestHeaders setObject:json_type   forKey:@"Accept"  ];
            ...
            PLog.i(CLASS_TAG, "// Request Http Header [" + httpHeadJson + "]");
	...
	public JSONObject setRequestHeader(String otherInfos, int packetSeq, NetReqOptions netReqOptions) {
		 ...
		 try {
			 ...
			 // device_id
             subHeadJson.put("device_id", Utils.getDeviceId(commHandle.getApplicationContext()));
...
//			 subHeadJson.put("callback_function", otherInfos);
             ...
             PLog.i(CLASS_TAG, "// Request Header [" + headJson + "]");
...
*/
			...
			try{
				if (sb != null) {
                    ...
                    PLog.i(CLASS_TAG, "// Request Data [" + sb + "]");
				}
			}catch(JSONException e){
				try{
					if (sb != null) {
...
//		String content_length = null;
		...
		try {
        	...
        	PLog.i(CLASS_TAG, "////////////////////////////////////////////////////////////////////////////////");
            PLog.i(CLASS_TAG, "// Response Http Header[" + httpHeadJsonObj + "]");
    ...
    public synchronized void getResponseData(Header[] hd, String rd, int packetSeq) {
        ...
        try {
            ...
            PLog.i(CLASS_TAG, "// Response Header[" + headDataJson + "]");
            ...
            // 전문 코드
            if(!recvHttpHeadJson.isNull("user_com_code"))
        ...
        if (selectedActivity != null) {
            ...
            String callBackFunc = HttpPacketManager.getInstance().getCallBackFunctionName(nRecvPacketId);

            // Requested network options
            NetReqOptions networkOptions = HttpPacketManager.getInstance().getNetworkOptions(nRecvPacketId);
            ...
            if (strResultCode.compareTo("200") == 0) {
                ...
                selectedActivity.responseData(IActivityNetworkable.RESPONSE_DATA_TYPE_HTTP, strTrCode, callBackFunc, bodyDataJson, networkOptions);
            } else {

                HttpPacketManager.getInstance().removeAllProgressDialog();
                if(selectedActivity instanceof MainActivity) {
