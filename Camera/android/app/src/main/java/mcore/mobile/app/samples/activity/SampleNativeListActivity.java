...
public class SampleNativeListActivity extends AbstractActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int layout = Utils.getDynamicID(thisObj,"layout","nativelist");
		requestWindowFeature(Window.FEATURE_NO_TITLE);
		setContentView(layout);

		int title_before = Utils.getDynamicID(thisObj,"id","title_before");
		Button btn = (Button)findViewById(title_before);
		...
		int weblist = Utils.getDynamicID(thisObj,"id","weblist");
		Button btnWeblist = (Button)findViewById(weblist);
		...
		int register = Utils.getDynamicID(thisObj,"id","register");
		Button btnView = (Button)findViewById(register);
...
//		PLog.i(CLASS_TAG, "Abstract onNewIntent()");
	...
	public void handlingError(String callerServerName, String trCode, String errCode, String errMessage, NetReqOptions netReqOpt) {
		int handling_error = Utils.getDynamicID(thisObj,"id","handling_error");
		EditText handlingError = (EditText)findViewById(handling_error);
	...
	public void responseData(int nDataType, String stTrCode, String otherInfos, String dhRecvData, NetReqOptions netReqOpt) {
		...
		if(dhRecvData != null && dhRecvData.length() > 0){
			...
			try {
				...
				int listitem = Utils.getDynamicID(thisObj,"layout","listitem");
				mItemResourceId = listitem;
				mAdapter = new ListAdapter<String[]>(this, mItemResourceId, mArray) {
					...
					public View getView(final int position, View convertView, ViewGroup parent) {
						...
						int list_item = Utils.getDynamicID(thisObj,"id","list_item");
						((LinearLayout)convertView.findViewById(list_item)).setOnClickListener(new View.OnClickListener() {
							@Override
						...
						int list_name = Utils.getDynamicID(thisObj,"id","list_name");
						int list_date = Utils.getDynamicID(thisObj,"id","list_date");
						int listlocation = Utils.getDynamicID(thisObj,"id","list_location");

						((TextView)convertView.findViewById(list_name)).setText(mArray.get(position)[0]);
						((TextView)convertView.findViewById(list_date)).setText(mArray.get(position)[1]);
						TextView list_location = findViewById(listlocation);
						list_location.setText(mArray.get(position)[2]+ "\n" + mArray.get(position)[3]);
				...
				int listView = Utils.getDynamicID(thisObj,"id","listView");
